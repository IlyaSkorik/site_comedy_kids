        ...
        .then(programs => {
            ...
            programs.forEach(program => {
                ...
                card.innerHTML = `
                    ...
                    <button class="btn bg-primary text-bg text-center hover:translate-y-[-3px] hover:shadow-[0_20px_80px_-10px_varprimary] hover:animate-fade-in-up-delayed" data-action="open-modal">${program.btnText}</button>
                    </div>
                `;
    ...
    fetch('/data/teachers.json?t=' + Date.now())
        ...
        .then(teachers => {
            ...
            teachers.forEach(teacher => {
                const card = document.createElement('div');
                card.className = "relative w-full xl:w-[350px] fullHD:w-[450px] 2K:w-[700px] h-[450px] fullHD:h-[600px] 2K:h-[1000px] rounded-(--border-radius-md) 2K:rounded-4xl overflow-hidden shadow-[0_15px_30px_rgba(0,0,0,0.4)]";
                card.innerHTML = `
