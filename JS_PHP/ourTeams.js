...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createTeamSlides() {
        ...
        prevSlide.className = 'absolute w-[350px] h-[500px] right-0 lg:right-[100%] scale-80 rounded-2xl overflow-hidden grayscale-60 brightness-90 opacity-70 shadow-[0_4px_15px_rgba(0,0,0,0.3)] transition-all duration-500 ease cursor-pointer z-[3] bottom-12';
        prevSlide.id = 'teamSlidePrev';

        const activeSlide = document.createElement('div');
        activeSlide.className = 'team-slide-active absolute w-[350px] h-[500px] left-[50%] translate-x-[-50%] rounded-2xl overflow-hidden transition-all duration-500 ease cursor-pointer z-[4] bottom-12';
        activeSlide.id = 'teamSlideActive';

        const nextSlide = document.createElement('div');
        nextSlide.className = "absolute w-[350px] h-[500px] left-0 lg:left-[100%] scale-80 rounded-2xl overflow-hidden grayscale-60 opacity-70 brightness-90 shadow-[0_4px_15px_rgba(0,0,0,0.3)] transition-all duration-500 ease cursor-pointer z-[3] bottom-12";
    ...
    const achievementsHtml = isActive && Array.isArray(data.achievements) 
        ? `<div class="absolute p-4 w-full flex gap-2 justify-center flex-wrap">
            ${data.achievements.map(ach => 
                `<span class="bg-[var(--accent)] px-4 py-1.5 rounded-2xl text-xs flex items-center gap-1 transition-all duration-300 ease">
    ...
    return `
        <div class="team-card relative w-full h-full bg-[var(--bg)] rounded-[var(--border-radius-md)] overflow-hidden shadow-[0_10px_25px_var(--color-shadow)] transition-all duration-300 ease flex flex-col">
            ...
            <div class="team-info absolute bottom-0 left-0 right-0 p-6 bg-[linear-gradient(transparent,#270e3490,#270e34bd,#270e34bd,#270e34bd,#270e34c8,#270e34cc,#270e34df,#270e34)] rounded-b-[var(--border-radius-md)] transition-transform duration-300 ease ${isActive ? 'opacity-100' : 'opacity-0'} ">
                <p class="text-sm leading-[1.6] mb-3.5">${data.description}</p>
            </div>
        </div>
    `;
}
